...
const Sidebar = () => {
  return (
    <aside className="w-72 bg-gray-50 border-r border-gray-200 flex flex-col p-4 transition-all duration-300">
      <div className="flex-shrink-0">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-bold text-lg">Chanakya AI.</span>
          </div>
        ...
        <div className="flex items-center space-x-3 p-2 rounded-lg mb-4">
          <img src="https://placehold.co/40x40/E2E8F0/4A5568?text=JC" alt="User Avatar" className="w-10 h-10 rounded-full" />
          <div className="flex-1">
            <p className="font-semibold text-sm">Julia Chang</p>
            <p className="text-xs text-gray-500">julia.chang@gmail.com</p>
          </div>
...
const SidebarNavItem = ({ item }) => (
    <li>
      <a href="#" className={`flex items-center space-x-3 p-2 rounded-md text-sm font-medium transition-all duration-200 ${item.active ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-gray-100 hover:scale-105'}`}>
        <item.icon size={20} />
...
const SidebarNav = () => {
  const navItems = [
    ...
    { icon: Users, label: 'Community', new: true },
  ];
...
const HistoryList = () => {
  const todayItems = [
    { icon: MessageSquare, label: 'Product-market fit analysis' },
    { icon: Code, label: 'Technical architecture review' },
    { icon: ImageIcon, label: 'Business model optimization' },
  ];

  const yesterdayItems = [
    { icon: Code, label: 'Market research strategy' },
    { icon: MessageSquare, label: 'Customer acquisition plan' },
    { icon: MessageSquare, label: 'Funding pitch preparation' },
    { icon: FileText, label: 'Operational efficiency audit' },
  ];

  return (
    <div>
      <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 px-2">Today</h3>
      <ul className="space-y-1 mb-4">
        {todayItems.map((item, index) => (
          <HistoryItem key={index} icon={item.icon} label={item.label} />
        ))}
      </ul>
      <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 px-2">Yesterday</h3>
      <ul className="space-y-1 mb-4">
        {yesterdayItems.map((item, index) => (
          <HistoryItem key={index} icon={item.icon} label={item.label} />
        ))}
      </ul>
      <button className="flex items-center space-x-2 text-sm text-gray-600 hover:text-gray-900 w-full p-2 rounded-md hover:bg-gray-100 transition-colors duration-200">
...
const HistoryItem = ({ icon: Icon, label }) => (
  <li>
    <a href="#" className="flex items-center space-x-3 p-2 rounded-md text-sm text-gray-600 hover:bg-gray-100 hover:scale-105 transition-all duration-200 truncate">
